...
aplot + 
  ...
  annotate(geom = 'text', 
           label = dfCounts[dfCounts$`Number Of Crimes` == max(dfCounts$`Number Of Crimes`), ]$Date, size = 4.2,
  ...
  annotate(geom = 'text', 
           label = dfCounts[dfCounts$`Number Of Crimes` == min(dfCounts$`Number Of Crimes`), ]$Date, size = 4.2,
...
tabTypeDF[tabTypeDF$Types == 'OTHER OFFENSE', 2] <- sum(sum(tabTypeDF[tabTypeDF$Frequency < 1000,]$Frequency), 
                                                        tabTypeDF[tabTypeDF$Types == 'OTHER OFFENSE',]$Frequency)
